...
from canva import Canva, AsyncCanva, DefaultAioHttpClient
from canva._utils import is_dict
...
logging.getLogger("canva").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Canva]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Canva(base_url=base_url, username=username, password=password, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncCanva]:
